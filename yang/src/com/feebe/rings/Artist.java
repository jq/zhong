        ...
        mTopTracksList.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView parent, View v, int position, long id) {    
				if(position > 1){
					...
					//intent.putExtra("key", mTopTracks.get(position - 2));
	                //intent.setClass(Artist.this, SearchList.class);
					//startActivity(intent);	
				    String artist = mArtistName;
				    String title = mTopTracks.get(position - 2);
				    Search.getArtistAndTitle(Artist.this, artist, title);
				}
            }
        });
