...
public final class ConnectionChecker implements Runnable {
    ...
    public static ConnectionChecker checkForLiveConnection() {
        LOG.trace("ConnectionChecker checking for live connection");
    ...
    private void connectToHost(String host) {
        ...
        HttpHead head = new HttpHead(host);
        head.addHeader("Cache-Control", "no-cache");
        head.addHeader("User-Agent", CommonUtils.getHttpServer());
        head.addHeader(HTTPHeaderName.CONNECTION.httpStringValue(),
            "close");
