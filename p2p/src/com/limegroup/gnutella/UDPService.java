...
public class UDPService implements ReadHandler, WriteHandler {
	...
	public void handleRead() throws IOException {
        while(true) {
            LOG.debug("UDPService handleRead");
            BUFFER.clear();
	    ...
	    synchronized(this) {     	
	        ...
	        if (LOG.isTraceEnabled()) {
	            LOG.trace("stable "+_portStable+
	                    ...
	                    " our non-forced port "+RouterService.getAcceptor().getPort()+
	        ...
	        if (_numReceivedIPPongs == 1){
	            ret = ret &&
	            	(_lastReportedPort == RouterService.getAcceptor().getPort() ||
	...
	public int getStableUDPPort() {
	    ...
	    int localPort = RouterService.getAcceptor().getPort();
