...
public class Base32 {
    ...
    static public void main(String[] args) {
        if (args.length==0) {
            LOG.info("Supply a Base32-encoded argument.");
            return;
        }
        LOG.info(" Original: "+args[0]);
        byte[] decoded = Base32.decode(args[0]);
        LOG.info  ("      Hex: ");
        for(int i = 0; i < decoded.length ; i++) {
            int b = decoded[i];
            if (b<0) b+=256;
            LOG.info((Integer.toHexString(b+256)).substring(1));
        }
        LOG.info();
        LOG.info("Reencoded: "+Base32.encode(decoded));
    }
    */
}
