...
public class GetHostsFromCache extends Thread {
	...
	public void run() {
		...
		while (!shutdownFlag) {
			...
			try {
				...
				// // LOG.error(					"Malformed URL in Default GWebCache list: "					+ (((String)urls.elementAt(position)).split(" "))[0]);
			...
			if (!error) {
				try {
					...
					// LOG.info(						"Getting input stream for "						+ (((String)urls.elementAt(position)).split(" "))[0]);
					incomingData =
			...
			if (!error) {
				// LOG.info(					"Adding hosts from GWebCache "					+ (((String)urls.elementAt(position)).split(" "))[0]					+ " to hostCache");
	...
	private void getCacheAddresses(String knownGoodCache) {
		...
		if (!error) {
			// LOG.info(				"Adding cache URLs from GWebCache " + knownGoodCache + " to hostCache");
